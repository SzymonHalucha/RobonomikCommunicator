...
class RobonomikCommunicatorHotReload(MDApp):
    ...
    def generate_application_views(self) -> MyRootWidget:
        ...
        for name in views.data:
            ...
            root.add_desktop_view(views.data[name]["desktop"](name=name, root=root, dialoger=self.dialoger, presenter=presenter))
            root.add_tablet_view(views.data[name]["tablet"](name=name, root=root, dialoger=self.dialoger, presenter=presenter))
            root.add_mobile_view(views.data[name]["mobile"](name=name, root=root, dialoger=self.dialoger, presenter=presenter))
        return root
