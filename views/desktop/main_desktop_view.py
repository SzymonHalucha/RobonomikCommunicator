...
#             self._presenter.on_preset_create(content.edited, lambda success: self.dialoger.close_dialogs() if success
#                                              else self.dialoger.show_text_field_error("preset_name", "Preset name is not valid"))
#             self.update_current_subview()
#         self.dialoger.open_confirm_dialog("Create Preset", MyCreatePresetDialogContent(), on_confirm)

#     def on_preset_select(self):
#         self.dialoger.open_list_dialog("Select Preset", self._presenter.on_preset_select(self.update_current_subview))

#     def on_preset_delete(self):
#         def on_delete():
#             self._presenter.on_preset_delete(self.dialoger.close_dialogs())
#             self.update_current_subview()
#         self.dialoger.open_delete_dialog("Delete Preset", "Are you sure you want to delete this preset?", on_delete)
